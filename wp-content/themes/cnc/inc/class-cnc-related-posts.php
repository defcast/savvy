...
if ( ! function_exists( 'CNC_call_related_posts_class' ) ) {
	/**
	 *
	 ...
	 *
	 */
	function CNC_call_related_posts_class() {
		$display_related_blog_posts = get_theme_mod( 'related_posts_area', true );

		if ( $display_related_blog_posts ) {

			// instantiate the class & load everything else
			CNC_Related_Posts::get_instance();
		}
	}

	add_action( 'wp_loaded', 'CNC_call_related_posts_class' );
}


if ( ! class_exists( 'CNC_Related_Posts' ) ) {
	...
	class CNC_Related_Posts {
		...
		protected function __construct() {
			...
			if ( $related_posts ) {
				add_action( 'CNC_single_after_article', array( $this, 'output_related_posts' ), 2 );
			}
		...
		function output_related_posts() {
			...
			echo '<div class="CNC-related-posts">';

			// Number of posts to show / view
			$limit      = get_theme_mod( 'CNC_howmany_blog_posts', 3 );
			$show_title = get_theme_mod( 'CNC_enable_related_title_blog_posts', true );
			$show_date  = get_theme_mod( 'CNC_enable_related_date_blog_posts', false );
			$auto_play  = get_theme_mod( 'CNC_autoplay_blog_posts', true );
			 ...
			 * Heading
			 */
			echo '<div class="col-lg-11 col-sm-10 col-xs-12 CNC-related-posts-title">';
			if ( is_singular( 'jetpack-portfolio' ) ) {
				echo '<h3><span>' . esc_html__( 'Related projects', 'CNC' ) . '</span></h3>';
			} else {
				echo '<h3><span>' . esc_html__( 'Related articles ', 'CNC' ) . '</span></h3>';
			}
			echo '</div>';
			 ...
			 * Arrows
			 */
			echo '<div class="CNC-carousel-navigation hidden-xs">';
			echo '<ul class="CNC-carousel-arrows clearfix">';
			echo '<li><a href="#" class="CNC-owl-prev fa fa-angle-left"></a></li>';
			echo '<li><a href="#" class="CNC-owl-next fa fa-angle-right"></a></li>';
			echo '</ul>';
			echo '</div>';
			...
			while ( $related_posts->have_posts() ) {
				...
				if ( has_post_thumbnail( $related_posts->post->ID ) ) {
					echo '<a href="' . esc_url( get_the_permalink() ) . '" class="related-item-thumbnail" style="background-image: url( ' . get_the_post_thumbnail_url( $related_posts->post->ID, 'CNC-grid' ) . ' )">' . get_the_post_thumbnail( $related_posts->post->ID, 'CNC-grid' ) . '</a>';
				} else {
				...
				if ( $show_title ) {
					echo '<div class="CNC-related-post-title">';
				...
				if ( $show_date ) {
					...
					echo '<div class="CNC-related-posts-date">';

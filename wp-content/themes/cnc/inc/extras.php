...
function CNC_body_classes( $classes ) {
	...
	if ( get_theme_mod( 'CNC_sidebar_position' ) == 'pull-right' ) {
		$classes[] = 'has-sidebar-left';
	} elseif ( get_theme_mod( 'CNC_sidebar_position' ) == 'no-sidebar' ) {
		$classes[] = 'has-no-sidebar';
	} elseif ( get_theme_mod( 'CNC_sidebar_position' ) == 'full-width' ) {
 ...
 * @return array
 */
function CNC_page_menu_args( $args ) {
	$args['show_home'] = true;

	return $args;
}

add_filter( 'wp_page_menu_args', 'CNC_page_menu_args' );

// Mark Posts/Pages as Untiled when no title is used
add_filter( 'the_title', 'CNC_title' );

function CNC_title( $title ) {
	if ( '' == $title ) {
		return esc_html__( 'Untitled', 'CNC' );
	} else {
		return $title;
	}
}
 ...
 */
add_filter( 'the_password_form', 'CNC_custom_password_form' );

function CNC_custom_password_form() {
	global $post;
	$label = 'pwbox-' . ( empty( $post->ID ) ? rand() : $post->ID );
	$o     = '<form class="protected-post-form" action="' . get_option( 'siteurl' ) . '/wp-login.php?action=postpass" method="post">
  <div class="row">
    <div class="col-lg-10">
        <p>' . esc_html__( 'This post is password protected. To view it please enter your password below:', 'CNC' ) . '</p>
        <label for="' . esc_attr( $label ) . '">' . esc_html__( 'Password:', 'CNC' ) . ' </label>
      <div class="input-group">
        <input class="form-control" value="' . esc_attr( get_search_query() ) . '" name="post_password" id="' . esc_attr( $label ) . '" type="password">
        <span class="input-group-btn"><button type="submit" class="btn btn-default" name="submit" id="searchsubmit" value="' . esc_attr__( 'Submit', 'CNC' ) . '">' . esc_html__( 'Submit', 'CNC' ) . '</button>
        </span>
      </div>
    </div>
  </div>
...
add_filter( 'the_content', 'CNC_add_custom_table_class' );
function CNC_add_custom_table_class( $content ) {
	return preg_replace( '/(<table) ?(([^>]*)class="([^"]*)")?/', '$1 $3 class="$4 table table-hover" ', $content );
}

if ( ! function_exists( 'CNC_header_menu' ) ) :
	/**
	 * Header menu (should you choose to use one)
	 */
	function CNC_header_menu() {
 ...
 */
function CNC_footer_info() {
	printf( esc_html__( 'Theme by %1$s Powered by %2$s', 'CNC' ), '<a href="https://colorlib.com/" target="_blank" title="Colorlib">Colorlib</a>', '<a href="http://wordpress.org/" target="_blank" title="WordPress.org">WordPress</a>' );
}


if ( ! function_exists( 'CNC_get_theme_options' ) ) {
	/**
	 * Get information from Theme Options and add it into wp_head
	 */
	function CNC_get_theme_options() {

		echo '<style type="text/css">';

		if ( get_theme_mod( 'link_color' ) ) {
			echo 'a, .image-bg a, .contact-section .social-icons li a, a:visited, .footer .footer-credits a, .post-content .post-meta li a, .post-content .CNC-category a, .module.widget-handle i {color:' . esc_attr( get_theme_mod( 'link_color' ) ) . ' }';
		}
        ...
        .post-content .CNC-category a:hover, .post-content .CNC-category a:focus, .main-navigation .menu li:hover > a, .main-navigation .menu > li:hover:after { color: ' . esc_attr( get_theme_mod( 'link_hover_color' ) ) . ';}';
		}
		...
		if ( get_theme_mod( 'button_color' ) ) {
          ...
          .post-content .more-link { background:' . esc_attr( get_theme_mod( 'button_color' ) ) . ' !important; border: 2px solid' . esc_attr( get_theme_mod( 'button_color' ) ) . ' !important;}';
			echo '.CNC_home_parallax > section:not(.image-bg) .btn-white { color:' . esc_attr( get_theme_mod( 'button_color' ) ) . ' !important; border: 2px solid' . esc_attr( get_theme_mod( 'button_color' ) ) . ' !important; }';
		}
		if ( get_theme_mod( 'button_hover_color' ) ) {
          ...
          .post-content .more-link:hover, .btn:not(.btn-white):hover, .button:not(.btn-white):hover  { background: ' . esc_attr( get_theme_mod( 'button_hover_color' ) ) . ' !important; border: 2px solid' . esc_attr( get_theme_mod( 'button_hover_color' ) ) . ' !important;}';
			echo '.CNC_home_parallax > section:not(.image-bg) .btn-white:hover, .CNC_home_parallax > section:not(.image-bg) .btn-white:focus, .pagination span:not( .dots ), .pagination a:hover, .woocommerce-pagination ul.page-numbers span.page-numbers, .woocommerce nav.woocommerce-pagination ul li a:focus, .woocommerce nav.woocommerce-pagination ul li a:hover, .woocommerce nav.woocommerce-pagination ul li span.current { background-color: ' . esc_attr( get_theme_mod( 'button_hover_color' ) ) . ' !important; border-color: ' . esc_attr( get_theme_mod( 'button_hover_color' ) ) . ' !important;color: #fff !important; }';
...
add_action( 'wp_head', 'CNC_get_theme_options', 10 );
 ...
 */
function CNC_caption( $output, $attr, $content ) {
	if ( is_feed() ) {
		return $output;
	}

	$defaults = array(
		'id'      => 'CNC_caption_' . rand( 1, 192282 ),
		'align'   => 'alignnone',
		'width'   => '',
		'caption' => '',
	);
...
add_filter( 'img_caption_shortcode', 'CNC_caption', 10, 3 );

/**
 * Adds the URL to the top level navigation menu item
 */
function CNC_add_top_level_menu_url( $atts, $item, $args ) {
...
add_filter( 'nav_menu_link_attributes', 'CNC_add_top_level_menu_url', 99, 3 );

/**
 * Makes the top level navigation menu item clickable
 */
function CNC_make_top_level_menu_clickable() {
	if ( ! wp_is_mobile() ) { ?>
...
add_action( 'wp_footer', 'CNC_make_top_level_menu_clickable', 1 );

/*
 * Add Read More button to post archive
 */
function CNC_excerpt_more( $more ) {
	return '<div><a class="btn-filled btn" href="' . esc_url( get_the_permalink() ) . '" title="' . the_title_attribute(
		array(
			'echo' => false,
		)
	) . '">' . esc_html_x( 'Read More', 'Read More', 'CNC' ) . '</a></div>';
}

add_filter( 'excerpt_more', 'CNC_excerpt_more' );

/*
 * Pagination
 */
if ( ! function_exists( 'CNC_pagination' ) ) {
 ...
 * Search Widget
 */
function CNC_search_form( $form ) {
	$form = '<form role="search" method="get" id="searchform" class="search-form" action="' . esc_url( home_url( '/' ) ) . '" >
    <label class="screen-reader-text" for="s">' . esc_html__( 'Search for:', 'CNC' ) . '</label>
    <input type="text" placeholder="' . esc_html__( 'Type Here', 'CNC' ) . '" type="text" value="' . esc_attr( get_search_query() ) . '" name="s" id="s" />
    <button type="submit" class="searchsubmit"><i class="fa fa-search" aria-hidden="true"></i><span class="screen-reader-text">' . esc_attr__( 'Search', 'CNC' ) . '</span></button>
    </form>';

	return $form;
}

add_filter( 'get_search_form', 'CNC_search_form', 100 );


/*
 * Author bio on single page
 */
if ( ! function_exists( 'CNC_author_bio' ) ) {

	function CNC_author_bio() {
 ...
 */
function CNC_cb_comment( $comment, $args, $depth ) {
			...
			<div class="comment">
				...
				<div class="comment-date">
					<time datetime="2016-01-28T12:43:17+00:00">
						...
						printf( __( '%1$s at %2$s', 'CNC' ), get_comment_date(), get_comment_time() );
						?>
						</time>
						<?php
						edit_comment_link( esc_html__( 'Edit', 'CNC' ), '  ', '' );
					?>
				</div>
				<?php
				...
				<?php if ( '0' == $comment->comment_approved ) : ?>
					<p>
						<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'CNC' ); ?></em>
						<br/>
					</p>
				<?php endif; ?>
 ...
 * Reply button class
 */
function CNC_reply_link_class( $class ) {
 ...
 */
function CNC_custom_comment_form() {
	$commenter = wp_get_current_commenter();
	$req       = get_option( 'require_name_email' );
	$aria_req  = ( $req ? " aria-required='true'" : '' );
	$fields    = array(
		'author' => '<input id="author" placeholder="' . esc_html__( 'Your Name', 'CNC' ) . ( $req ? '*' : '' ) . '" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" ' . $aria_req . ' required="required" />',

		'email'  => '<input id="email" name="email" type="email" placeholder="' . esc_html__( 'Email Address', 'CNC' ) . ( $req ? '*' : '' ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' required="required" />',

		'url'    => '<input placeholder="' . esc_html__( 'Your Website (optional)', 'CNC' ) . '" id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" />',
	);

	$comments_args = array(
		'label_submit'  => esc_html__( 'Leave Comment', 'CNC' ),
		'comment_field' => '<textarea placeholder="' . _x( 'Comment', 'noun', 'CNC' ) . '" id="comment" name="comment" cols="45" rows="8" aria-required="true" required="required"></textarea>',
 ...
 * Header Logo
 */
function CNC_get_header_logo() {

	$logo_dimensions = get_theme_mod( 'CNC_logo_dimension', array() );
	...
	elseif ( is_customize_preview() ) {
		$html = sprintf(
			'<a href="%1$s" class="custom-logo-link"><img class="custom-logo"/ style="display:none;"><span class="site-title">%2$s</span></a>',
			esc_url( home_url( '/' ) ),
			esc_html( get_bloginfo( 'name' ) )
		);
	} else {
		$html = sprintf(
			'<a href="%1$s" class="custom-logo-link"><span class="site-title">%2$s</span></a>',
			esc_url( home_url( '/' ) ),
			esc_html( get_bloginfo( 'name' ) )
		);
	}
...
function CNC_show_sidebar() {
	...
	} elseif ( get_theme_mod( 'CNC_sidebar_position' ) == 'no-sidebar' || get_theme_mod( 'CNC_sidebar_position' ) == 'full-width' ) {
		$show_sidebar = false;
	}
				...
				<?php if ( $title_in_post ) : ?>
					<div class="<?php echo $breadcrumbs_enabled ? 'col-md-6 col-sm-6 col-xs-12' : 'col-xs-12'; ?>">
						<h3 class="page-title" <?php echo $header_color ? 'style="color:#' . esc_attr( $header_color ) . '"' : ''; ?>>
							<?php
							if ( is_home() ) {
								echo esc_html( get_theme_mod( 'blog_name' ) ? get_theme_mod( 'blog_name' ) : __( 'Blog', 'CNC' ) );
							} elseif ( is_search() ) {
								_e( 'Search', 'CNC' );
							} elseif ( is_archive() ) {
								if ( is_post_type_archive( 'jetpack-portfolio' ) ) {
									$portfolio_title = get_theme_mod( 'portfolio_name', esc_html__( 'Portfolio', 'CNC' ) );
...
// Check WooCommerce
if ( ! function_exists( 'CNC_is_woocommerce_activated' ) ) {
	function CNC_is_woocommerce_activated() {
